...
ValidInTransitProposeTS ==
  ...
  /\ \A snder, rcver \in Proc : \A m \in inTransit[snder][rcver] : 
          m.t = PType => ( /\ globalTS[m.dest][m.id] = TimestampNull
                           /\ ~(\E m1 \in inTransit[McastID[m.id]][m.source] : m1.dest = m.source /\ m1.id = m.id /\ m1.type = MType)
...
ValidRcvdProposeTS1 ==
  /\ ( \A id \in McastID : \A rcver \in Proc \ GroupDest[id] : \A snder \in Proc : 
          proposeTS[rcver][id] = ({} <: {[type |-> Int, t |-> Int, id |-> Int, source |-> Int, dest |-> Int]} ))
          
ValidRcvdProposeTS2 ==                    
  /\ ( \A id \in McastID : \A rcver \in GroupDest[id] : \A msg \in proposeTS[rcver][id] :
          /\ msg.t = localTS[msg.source][msg.id].t
          /\ (\A m \in inTransit[msg.source][rcver] : (m.source = msg.source /\ m.dest = rcver) => m.id # id ) )
...
UniqueMsg1 ==
  \A m1, m2 \in inTransit :
    /\ ( m1.source = m2.source /\ m1.dest = m2.dest /\ m1.type = m2.type /\ m1.id = m2.id) => m1.t = m2.t

UniqueMsg2 ==
  \A m1, m2 \in inTransit :    
    /\ ( m1.source = m2.source /\ m1.dest = m2.dest /\ m1.type = m2.type /\ m1.t  = m2.t)  => m1.id = m2.id

UniqueMsg3 ==
  \A m1, m2 \in inTransit :
    /\ ( m1.source = m2.source /\ m1.dest = m2.dest /\ m1.id   = m2.id   /\ m1.t  = m2.t)  => m1.type = m2.type
...
State0 ==
  clock = 1 :> 5 @@ 2 :> 4
    ...
    /\ globalTS
      = 1 :> (1 :> [g |-> 0, t |-> 0] @@ 2 :> [g |-> 1, t |-> 3])
        @@ 2 :> (1 :> [g |-> 0, t |-> 0] @@ 2 :> [g |-> 1, t |-> 3])
    /\ inTransit
      = { [dest |-> 1, id |-> 1, source |-> 1, t |-> 4, type |-> 11],
        [dest |-> 2, id |-> 1, source |-> 1, t |-> 4, type |-> 11],
        [dest |-> 2, id |-> 1, source |-> 1, t |-> 5, type |-> 10] }
    /\ localTS
      = 1 :> (1 :> [g |-> 1, t |-> 4] @@ 2 :> [g |-> 1, t |-> 3])
        @@ 2 :> (1 :> [g |-> 0, t |-> 0] @@ 2 :> [g |-> 2, t |-> 2])
    /\ mcastedID = { 1, 2 }
    /\ phase = 1 :> (1 :> 13 @@ 2 :> 14) @@ 2 :> (1 :> 12 @@ 2 :> 14)
    /\ proposeTS
      = 1
          :> (1 :> {}
            @@ 2
              :> {  [dest |-> 1, id |-> 2, source |-> 1, t |-> 3, type |-> 11],
                    [dest |-> 1, id |-> 2, source |-> 2, t |-> 2, type |-> 11],
                    [dest |-> 2, id |-> 2, source |-> 2, t |-> 2, type |-> 11] })
        @@ 2
          :> (1 :> {}
            @@ 2
              :> {  [dest |-> 1, id |-> 2, source |-> 1, t |-> 3, type |-> 11],
          ...
          :> (1 :> {}
            @@ 2
              :> { [dest |-> 1, id |-> 2, source |-> 1, t |-> 3, type |-> 11],
                ...
                [dest |-> 2, id |-> 2, source |-> 2, t |-> 2, type |-> 11] })
        @@ 2
          :> (1 :> {[dest |-> 2, id |-> 1, source |-> 1, t |-> 4, type |-> 11]}
            @@ 2
              :> { [dest |-> 1, id |-> 2, source |-> 1, t |-> 3, type |-> 11],
                [dest |-> 1, id |-> 2, source |-> 2, t |-> 2, type |-> 11],
                [dest |-> 2, id |-> 2, source |-> 2, t |-> 2, type |-> 11] })
...
\* Last modified Tue Mar 16 19:00:54 CET 2021 by tran
